 ...
 */
var postFilter = function (originalCommands) {
    var commands = [];
    var commandsToSkip = {
        'waitForPageToLoad':1,
        'pause':1
...
function formatHeader(testCase) {
    ...
    var formatLocal = testCase.formatLocal(options.name);
    var methodName = testMethodName(className.replace(/Test$/i, "").replace(/^Test/i, "").
    ...
    var header = (options.getHeader ? options.getHeader() : options.header).
        ...
        replace(/\$\{chromePath\}/g, options.chromePath).
        replace(/\$\{driver\}/g, options.driver).
        replace(/\$\{baseURL\}/g, testCase.getBaseURL()).
        replace(/\$\{([a-zA-Z0-9_]+)\}/g, function (str, name) {
            return options[name];
        });
    options.lastIndent = indents(parseInt(options.initialIndents, 10));
...
function addDeclaredVar(variable) {
    if (options.declaredVars == null) {
        options.declaredVars = {};
    }
    options.declaredVars[variable] = true;
}

function newVariable(prefix, index) {
    if (index == null) index = 1;
    if (options.declaredVars && options.declaredVars[prefix + index]) {
