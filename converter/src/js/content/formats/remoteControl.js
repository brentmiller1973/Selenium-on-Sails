...
function formatHeader(testCase) {
    ...
    var header = options.header.replace(/\$\{className\}/g, className).
        ...
        replace(/\$\{superClass\}/g, options.superClass).
...
function seleniumEquals(type, pattern, expression) {
    if (type == 'String[]') {
        ...
        if (separateEquals) {
            ...
            result.conditions.push(new Equals(list.length, { toString:function () {
                return result.length()
            }}));
            for (var i = 0; i < list.length; i++) {
                result.conditions.push(seleniumEquals('String', matcher + ':' + list[i],
                    { index:i, toString:function () {
                        return result.item(this.index)
                    }}));
            }
            return result;
        } else {
